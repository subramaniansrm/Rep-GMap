...
<body>
    <center>
        <h1>User Add / Update Management</h1>
        <h2>
    ...
    <div align="center">
     <%--    <c:if test="${u != null}">
        ...
        <c:if test="${u!= null}">
              <form:form  modelAttribute="user" method = "POST" action = "${pageContext.request.contextPath}/update"   >
            ...
            <caption>
                ...
                <c:if test="${u != null}">
                    <input type="hidden" name="userId" value="<c:out value='${u.userId}' />" />
                     <input type="hidden" name="password" value="<c:out value='${u.password}' />" />
